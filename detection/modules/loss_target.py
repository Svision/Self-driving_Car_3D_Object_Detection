...
def create_heatmap_RAG(grid_coords: Tensor, center: Tensor, scale: float, yaw: float, x_size: float, y_size: float) -> Tensor:
    # DONE: Replace this stub code.
    H, W, _ = grid_coords.shape
    cx = center[0]
    cy = center[1]
...
class DetectionLossTargetBuilder:
    ...
    def build_target_tensor_for_label(
        ...
        heatmap = create_heatmap_RAG(grid_coords, center=center, scale=self._heatmap_norm_scale, yaw=yaw, x_size=x_size, y_size=y_size)  # [H x W]
        ...
        filtered_grid_coords = grid_coords[heatmap > self._heatmap_threshold]
        offsets[heatmap > self._heatmap_threshold] = center - filtered_grid_coords
